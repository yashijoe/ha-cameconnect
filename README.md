...
Home Assistant
  └── ha-cameconnect
        └── CAME Connect Cloud API (app.cameconnect.net / beta.cameconnect.net)
...
| Username | Your CAME Connect email |
...
To add a second gate, run the integration setup again. Credentials are re-used; only the Device ID and name need to change.

---

## Entities

For each configured device, two entities are created under the same Device card:
...
| `maneuvers` | Total maneuver counter (from device States/CommandId=18) |
...
→ Double-check Client ID, Client Secret, email and password. The integration tests credentials live — if it fails here, the API rejected them.

**State always `unknown`**  
→ Verify the Device ID. You can check the raw API response via HA logs (enable debug: `logger: logs: custom_components.ha_cameconnect: debug`).
...
Original proxy add-on by [@yashijoe](https://github.com/yashijoe).  
